...
cmd = f"docker run -v {SERVER_VOLUME_NAME}:/etc/openvpn -it --rm kylemanna/openvpn ovpn_genconfig -u {CONNECT_METHOD}://{SERVER_ADDR}:{CONNECT_PORT} -e 'duplicate-cn' -d -D -b -c -s {VPN_SUBNET}"
